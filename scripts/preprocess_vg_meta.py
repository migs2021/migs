...
# Input data
parser.add_argument('--splits_json', default='./sg2im/data/vg_meta_splits.json')
...
parser.add_argument('--clusters',
    default=os.path.join(VG_DIR, 'pred_clusters_vg.json'))
...
parser.add_argument('--output_h5_dir', default=os.path.join(VG_DIR, 'Meta_h5'))
...
def main(args):
  ...
  numpy_arrays = encode_graphs(args, splits, objects, relationships, vocab,
                               object_id_to_obj, attributes, cluster_assignments, image_id_to_image)
...
def encode_graphs(args, splits, objects, relationships, vocab,
  ...
  for split, image_ids in splits.items():
    ...
    object_attributes = []
    for image_id in image_ids:
      #cluster_id = cluster_assignments[str(image_id) + '.jpg']
      #print(image_id, cluster_id)
      image_object_ids = []
      image_object_names = []
      image_object_boxes = []
      object_id_to_idx = {}
