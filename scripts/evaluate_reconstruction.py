...
from sg2im.loaders_utils import build_eval_loader
...
def eval_model(model, loader, device, vocab, use_gt_boxes=False, use_feats=False, filter_box=False):
    ...
    with torch.no_grad():
        for batch in tqdm.tqdm(loader):
            ...
            imgs, objs, boxes, triples, obj_to_img, triple_to_img = [b.to(device) for b in batch]
            ...
            if num_batches % args.print_every == 0:
                calculate_scores(mae_per_image, total_iou,  ssim_per_image,
                                 perceptual_error_image)

            if num_batches % args.save_every == 0:
                save_results(mae_per_image, total_iou, ssim_per_image, perceptual_error_image, all_boxes, num_batches)

            img_idx += 1

    calculate_scores(mae_per_image, total_iou, ssim_per_image,
                                 perceptual_error_image)
    save_results(mae_per_image, total_iou, ssim_per_image,
                 perceptual_error_image, all_boxes, 'final')


def calculate_scores(mae_per_image, total_iou, ssim_per_image,
                     perceptual_image):
...
def save_results(mae_per_image, total_iou, ssim_per_image,
