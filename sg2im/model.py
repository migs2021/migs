...
from sg2im.stylegan2 import StyleGAN2
...
class Sg2ImModel(nn.Module):
    def __init__(self, vocab, image_size=(64, 64), embedding_dim=64,
                 ...
                 mask_size=None, mlp_normalization='none', layout_noise_dim=0, rank=0, spade_blocks=False,
                 layout_size=None, stylegan_use_random_noise=False, stylegan_latent_dim=256,
                 filters_from_capacity=False, use_noisy_layout=False, network_capacity=16, add_non_up=True, init_chan_user_inp=False,
                 use_crn = False,
                 **kwargs):
        ...
        if layout_size is not None:
            self.layout_dim = layout_size
        else:
            self.layout_dim = image_size
        ...
        if self.spade_blocks:
            ...
            refinement_kwargs = {
              'dims': (gconv_dim + layout_noise_dim,) + decoder_dims,
              'normalization': normalization,
              'activation': activation,
            }
            self.refinement_net = RefinementNetwork(**refinement_kwargs)
        elif not self.spade_blocks:
            self.styleGAN = StyleGAN2(self.image_size[0], layout_size, latent_dim=stylegan_latent_dim,
                                      initial_size=layout_size[0], network_capacity=network_capacity,
                                      layout_emb_depth=gconv_dim,
                                      noise_emb_depth=self.layout_noise_dim, rank=rank,
                                      use_random_noise=stylegan_use_random_noise,
                                      filters_from_capacity=filters_from_capacity, use_noisy_layout=use_noisy_layout,
                                      add_non_up=add_non_up, init_chan_user_inp = init_chan_user_inp)
    ...
    def forward(self, objs, triples, obj_to_img=None,
        ...
        H, W = self.image_size
        layout_H, layout_W = self.layout_dim
        layout_boxes = boxes_pred if boxes_gt is None else boxes_gt

        if masks_pred is None:
            if self.spade_blocks or self.use_crn:
                layout = boxes_to_layout(obj_vecs, layout_boxes, obj_to_img, H, W)
            else:
                layout = boxes_to_layout(obj_vecs, layout_boxes, obj_to_img, layout_H, layout_W)
        else:
        ...
        if self.spade_blocks:
            generated_images = self.decoder_net(layout)
        elif self.use_crn:
            generated_images = self.refinement_net(layout)
        else:
            # Style GAN network
            N, C, _, _ = layout.size()
            H, W = self.image_size
            image_noise = torch.FloatTensor(N, H, W, 1).uniform_(0., 1.).cuda(layout.device)
            layout_32_enc = boxes_to_layout(obj_vecs, layout_boxes, obj_to_img, 32, 32)
            # noise_shape = (N, self.layout_noise_dim, 32, 32)
            # layout_noise = torch.randn(noise_shape, dtype=layout.dtype,
            #                            device=layout.device)
            if self.use_noisy_layout:
                layout = torch.cat([layout, layout_noise], dim=1)
            generated_images = self.styleGAN(layout, layout_noise, image_noise, layout)
        return generated_images, boxes_pred, masks_pred, rel_scores
